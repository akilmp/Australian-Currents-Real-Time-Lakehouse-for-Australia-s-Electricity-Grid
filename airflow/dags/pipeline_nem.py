...
with Great Expectations, runs a Spark batch job to build the Silver layer on
MinIO/S3, validates the Silver layer, executes dbt models and tests, and
finally sends notifications.  Connections for Kafka, MinIO/S3 and Slack are
created automatically when the DAG is parsed so the pipeline can run in a fresh
local environment.
"""
